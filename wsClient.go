...
type WsClient struct {
	...
	callerHost string
	textMode string
	dialID string // only usefull in the callerClient
...
func serve(w http.ResponseWriter, r *http.Request, tls bool) {
	...
	*/
	...
	dialID := ""
	if client.dialID!="" && client.dialID!=client.calleeID {
		dialID = "/"+client.dialID
	}
...
func (c *WsClient) handleClientMessage(message []byte, cliWsConn *websocket.Conn) {
	...
	if cmd=="init" {
		...
		if !strings.HasPrefix(c.calleeID,"answie") && !strings.HasPrefix(c.calleeID,"talkback") {
			...
			// we remove all entries that are older than X min
			countOutdated:=0
	...
	if cmd=="callerOffer" {
		...
		dialID := ""
		if c.dialID!="" && c.dialID!=c.calleeID {
			dialID = "/"+c.dialID
		}
//		callerName := ""
...
//		}
		...
		if c.callerID!="" {
			mappingMutex.RLock()
			mappingData,ok := mapping[c.callerID]
			mappingMutex.RUnlock()
			if ok {
				callerMainID = mappingData.CalleeId
				if callerMainID==c.callerID {
					callerMainID = ""
				} else {
		...
		if c.hub.CalleeClient==nil {
			fmt.Printf("# %s (%s%s) CALL⚡ from (%s%s) %s but hub.CalleeClient==nil\n",
				c.connType, c.calleeID, dialID, callerMainID, c.callerID, c.RemoteAddr)
		...
		if c.hub.ConnectedCallerIp!="" {
			...
			fmt.Printf("# %s (%s%s) CALL⚡ but hub.ConnectedCallerIp not empty (%s) <- (%s%s) %s\n",
				c.connType, c.calleeID, dialID, c.hub.ConnectedCallerIp, callerMainID, c.callerID, c.RemoteAddr)
		...
		fmt.Printf("%s (%s%s) CALL⚡ %s <- %s (%s%s) T=%s v=%s ua=%s\n",
			c.connType, c.calleeID, dialID, c.hub.CalleeClient.RemoteAddr,
				c.RemoteAddr, callerMainID, c.callerID, c.textMode, c.clientVersion, c.userAgent)

		// forward the callerOffer message to the callee client
		err := c.hub.CalleeClient.Write(message)
		if err != nil {
			// callee is gone
			fmt.Printf("! %s (%s%s) CALL CalleeClient.Write(calleroffer) fail %v\n",
				c.connType, c.calleeID, dialID, err)
			c.hub.HubMutex.RUnlock()
		...
		// 'callerInfo' is highly important for the Android client we want to always send it
	...
	if cmd=="cancel" {
		...
		if c.hub.CalleeClient.isConnectedToPeer.Load() {
			...
			if c.isCallee && payload!="disconnectByCaller" {
				...
				fmt.Printf("%s (%s%s) REQ PEER DISCON %s by callee cancel='%s'\n",
					c.connType, c.calleeID, dialID, c.RemoteAddr, payload)
			...
			fmt.Printf("%s (%s%s) REQ PEER DISCON <- by caller cancel (%s) %s\n",
				c.connType, c.calleeID, dialID, payload, c.RemoteAddr)
			// end the peer-connection
	...
	if cmd=="pickupWaitingCaller" {
		...
		fmt.Printf("%s (%s) pickupWaitingCaller from %s (%s)\n", c.connType, c.calleeID, c.RemoteAddr, callerAddrPort)

		if c.hub.CallerClient!=nil {
			fmt.Printf("%s (%s) pickupWaitingCaller closeCaller\n", c.connType, c.calleeID)
		...
		if ok {
			fmt.Printf("%s (%s) pickupWaitingCaller accept via channel\n", c.connType, c.calleeID)
			chn <- 1 // accept
		} else {
			fmt.Printf("! %s (%s) pickupWaitingCaller channel gone\n", c.connType, c.calleeID)
		}
		return
	}

	if cmd=="rejectWaitingCaller" {
		// for callee only
		// payload = ip:port
		callerAddrPort := payload
			...
			if len(waitingCallerSlice)>0 {
				for idx := range waitingCallerSlice {
					if waitingCallerSlice[idx].AddrPort == callerAddrPort {
						...
						if err != nil {
							fmt.Printf("# %s (%s) rejectWaitingCaller failed to remove (%s) from dbWaitingCaller\n",
								c.connType, c.calleeID, callerAddrPort)
						}
						break
					} else {
	...
	if cmd=="pickup" {
		...
		if c.pickupSent.Load() {
			...
			fmt.Printf("! %s (%s) skip pickup, already sent %s\n",
				c.connType, c.calleeID, c.RemoteAddr)
			return
		}
...
//		}
		if tok[0]=="callee" {
			if(constateShort=="RING") {
				...
				fmt.Printf("%s (%s%s) PEER callee %s☎️  %s %s <- %s (%s%s)\n",
					c.connType, c.calleeID, dialID, constateShort, p2pRelayedInfo,
					c.hub.CalleeClient.RemoteAddrNoPort, c.hub.CallerIpNoPort,
					callerMainID, c.hub.CallerID)
			}
			...
			if strings.HasPrefix(constate,"Con") && !c.isConnectedToPeer.Load() {
				fmt.Printf("%s (%s%s) PEER caller %s☎️  %s %s <- %s (%s%s)\n",
					c.connType, c.calleeID, dialID, constateShort, p2pRelayedInfo,
					c.hub.CalleeClient.RemoteAddrNoPort, c.hub.CallerIpNoPort,
					callerMainID, c.hub.CallerID)
			} else {
				fmt.Printf("%s (%s%s) PEER caller %s %s %s <- %s (%s%s)\n",
					c.connType, c.calleeID, dialID, constateShort, p2pRelayedInfo,
					c.hub.CalleeClient.RemoteAddrNoPort, c.hub.CallerIpNoPort,
					callerMainID, c.hub.CallerID)
			}
